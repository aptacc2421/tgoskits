...
/// (`-netdev user,hostfwd=tcp::<host_port>-:<guest_port>`). The probe itself is
/// the typed [`crate::axvisor::test::http_probe`] module that makes real HTTP
/// requests with `reqwest` and asserts the responses entirely host-side; the
/// runner only orchestrates (forward the port, run the probe, report its
/// result). This config is AxVisor-specific: it carries the bearer token and
/// timeouts the management API asserts on, so it lives in the AxVisor test
/// layer rather than the generic test layer.
