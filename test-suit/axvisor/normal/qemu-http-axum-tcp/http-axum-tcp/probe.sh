#!/bin/sh
# Host-side probe for the axum read-only management API over real TCP
# (qemu-http-axum-tcp). Same contract as http-axum-readonly: the in-guest
# management server is dialed through QEMU user-mode networking hostfwd, and the
# assertions run entirely host-side:
