...
| `Destroying` | `Destroying` | 无 | 无 | `take_resources_for_destroy` 的入口占位（machine.rs:500）；每个分支随即改写为 `Destroyed` 或还原原状态，锁释放后不可观测（§4） |
...
vm 层先 `take_stopped_runtime`（vm/mod.rs:1762）把旧 runtime 清掉，再调 `start_with`
...
`None`（幂等）；vm 层调用点：`stop_and_join_runtime` 尾部（vm/mod.rs:1962-1964，取走后
`join_all_vcpu_tasks`）与 start-from-Stopped（vm/mod.rs:1762-1764）。**调用者应检查返回值**：第二次
...
`Running` 的语义是"runtime 已创建、vCPU task 已入队（vm/mod.rs:1789 的
...
> **`start_with` 的闭包提交 runtime `H`**（`VmRuntimeHandle::new()` 在 vm/mod.rs:1772，闭包在
> :1811-1814），但它只接受
> `Stopped{runtime: None}`——`{runtime: Some}` 返回 `InvalidTransition`（machine.rs:142-157）。
> 因此 vm 层 `start` 会先 `take_stopped_runtime`（vm/mod.rs:1762）清空 runtime，再调
> `start_with`（详见 §3）。
...
`take_resources_for_destroy`（machine.rs:499-579）是 destroy 的 machine 层入口：它只做状态提交
...
- `Failed(_)` / `Destroying` / `Switching`（无可交出的 resources，返回 `Ok(None)`）——其中
  `Destroying`/`Switching` 是**防御性**分支（正常锁语义下不可达，见 §4）；
- `Destroyed`（幂等，machine.rs:502-505）。

它**拒绝** `Running`/`Pausing`/`Paused`/`Stopping`/`Stopped { runtime: Some }`
（machine.rs:510-565）。因此运行态 destroy 是**两步**：vm 层先
`stop_and_join_runtime(Forced)` **阻塞**等 vCPU 退出、`take_stopped_runtime` 清 runtime
（vm/mod.rs:2475-2504），再调 `take_resources_for_destroy`。`Stopping` 状态由 vm 层兜底（machine
层对 `Stopping` 的入口拒，:534-549）：`stop_and_join` 对 `Stopping` 直接 notify 并等
...
立刻写入 `Machine::Destroyed`（machine.rs:500/:503/:507/:571/:575），再把 resources 经返回值交给
调用者；调用者在 machine 锁释放后才执行清理（`AxVM::cleanup_resource_set`，vm/mod.rs:2506）。这样
...
resources 也已交出并随局部变量 drop。第二个（并发或重入）`destroy()` 命中 `Destroyed` 快速路径
直接返回 `Ok(())`，既不等待第一次清理结束、也不重做清理（vm/mod.rs:2499-2502）。因此调用者不应把
第二个 `Ok(())` 当作"清理已完成"的证据；旧实现里"清理失败后卡在 `Destroying` 供重试"的路径已不
存在。

**`stop_and_join_runtime(reason)` 集中描述**（vm/mod.rs:1928-1966；**全程不持有 machine 锁**，各子步
单次取锁）：
1. `status()` 分派：`Running`/`Paused` → `stop(reason)`（`request_stop_with` → `Stopping`）+
   ...
   （vm/mod.rs:2481-2484），对 `Destroyed` 直接跳过等待（:2489），均不经 `stop_and_join_runtime`。
...
`reset_with`（machine.rs:402-489）同样**只接受** `Ready`（幂等，:408）与
`Stopped { resources: Some, runtime: None }`（:413）；`Running`/`Paused`/`Stopping`/
`Stopped { runtime: Some }` 一律 `InvalidTransition`（:431-478）。`Stopped { resources: None }`
属类型级预留形态，落 `other` 兜底 → `InvalidTransition`（:479-487）。vm 层 `reset`
（vm/mod.rs:1970-1987）对运行态先强制静默到 `Stopped`，再走 `reset_with` → `Ready` 内部瞬态 →
...
join 会以 `TaskError::UnsafeContext` 失败并最终 abort。`AxVM::reset`（vm/mod.rs:1974-1984）与
...
| `Switching` | **转换入口占位**：每个转换函数入口第一行都 `core::mem::replace(self, Machine::Switching)`（`start_with` :112、`pause` :171、`resume` :190、`stop_with` :212、`request_stop_with` :279、`finish_stop` :347、`reset_with` :406、`take_resources_for_destroy` :500），执行期间锁一直被持有，`status()` 拿不到锁 → 不可观测。`Machine::status()` 对 `Switching` 的映射是**显式分支** `Switching => VmStatus::Failed`（machine.rs:48），不是 `_ =>` 兜底——这是有意的：万一 `Switching` 泄漏（持锁逻辑出错），外部只会看到 `failed` 而非崩溃。**设计权衡**：把不可观测占位映射为 `Failed`，使泄漏可被调用方当作"转换失败"识别；代价是调用方无法区分泄漏与真实失败 | machine.rs:112/:171/:190/:212/:279/:347/:406/:500；:48 |
| `Destroying` | **入口占位，锁内即被改写**：`take_resources_for_destroy` 第一行 `replace(self, Machine::Destroying)`（machine.rs:500）只是取走 `self` 的手段；每个接受分支都在同一临界区内立刻写入 `Machine::Destroyed`，拒绝分支把原状态写回，因此既拿不到锁、也不留残余可观测。**与旧实现不同，它不再有可达的泄漏路径**：状态在清理开始前就已提交为 `Destroyed`，清理失败只经返回值报告，不改变状态（§3.3） | machine.rs:500、:503/:507/:571/:575 |

**锁语义：** `status()`（vm/mod.rs:1470）与所有转换方法走**同一把 machine lock**——
...
**确实失败后**的 `Failed`（machine.rs:120/:217 的闭包失败路径），以及 **状态已提交为
`Destroyed`、但清理失败** 的情形（`AxVM::destroy` 返回 `Err`；§3.3）。转换方法的闭包内任何对
`status()`/`resources()` 的再调用都会在同一锁上重入而死锁（§4.1 第 1 条）。

`wait_until_stopped`（vm/mod.rs:1906-1926）在 machine 锁**外**执行：每次迭代 `status()` 单次取锁、
...
- **`destroy()` 不会把 VM 移出 manager 注册表**（`AxVM::destroy` 只释放资源，vm/mod.rs:2475-2504；
...
| `Stopping → reset` | `reset_with` :431 | machine 层 `InvalidTransition`；vm 层 `reset` 会先经 `stop_and_join_runtime` 等 vCPU 退出到 `Stopped` 再继续（vm/mod.rs:1972） |
| `Stopping → destroy` | vm 层 :2478（machine 层 :534 拒） | `vm::destroy` 允许：对 Stopping 直接 notify + 等 vCPU 退出（stop_and_join）再销毁 |
...
| `Destroyed → destroy` | `take_resources_for_destroy` :502 | 幂等成功（`destroy` 对 `Destroyed` 直接 Ok） |
| `Running → Paused` | `machine::pause` :170 | 同步；**不 notify、也不主动 park** vCPU——vCPU 在下一次 VM-exit 看到 `suspending()` 后自行 `wait_for(!suspending)`（vcpus.rs:637-645）。锁语义：`vm::pause` 在整个 `suspend_lifecycle_devices()` + `machine.pause()` 期间持锁（vm/mod.rs:1860-1872），外部 `status()` 此时阻塞。这是**设计意图**——设备挂起与状态翻转原子化，避免 `Paused` 已可见但设备仍在 DMA 的窗口。代价：若设备挂起实现引入慢路径（如阻塞 I/O），持锁时间会从微秒级膨胀，阻塞同一 VM 上所有 `status()`/生命周期操作并屏蔽本地中断（§4）；若需把 `suspend_lifecycle_devices()` 移到锁外，须先处理挂起期间 `stop()`/`resume()` 并发的竞态 |
| `Paused → Running` | `resume` :189 + `notify_all_vcpus`（runtime/vcpus.rs:96） | 唤醒 park 的 vCPU |
...
| `* → destroy`（运行态） | `vm::destroy` vm/mod.rs:2475 | 先强制静默：`stop_and_join_runtime(Forced)` 阻塞等 vCPU 退出，再 destroy |
| `Ready/Stopped(runtime:None)/Failed → Destroyed` | `take_resources_for_destroy` machine.rs:499 | 直接销毁（`Stopped` 带 runtime 时拒绝，须先 `take_stopped_runtime`） |
| `Ready/Stopped/运行态 → Running`（reset） | `vm::reset` vm/mod.rs:1970；`reset_with` machine.rs:402 | 分步：运行态先强制静默(→Stopped) → `reset_with`(→Ready，仅接受 `Ready`/`Stopped{runtime:None}`，machine.rs:408/:413) → prepare → start(→Running)。`Failed`/`Destroyed` 不可 reset（machine.rs:479 兜底拒） |
...
`request_stop_with`（machine.rs:275）是**运行时实际使用的路径**（vm 层 `stop` 调它，vm/mod.rs:1894），
...
| 公开方法 | 定义位置 |
|---------|---------|
| `AxVM::new` / `start` / `pause` / `resume` / `stop` / `reset` / `destroy` | `src/vm/mod.rs`（new :1435, start :1760, pause :1860, resume :1876, stop :1892, reset :1970, destroy :2475） |
| `AxVM::status` / `running` / `stopping` / `suspending` / `stopped` | `src/vm/mod.rs`（status :1470, running :1840, stopping :1845, suspending :1850, stopped :1855） |
...
`crate::config::init_guest_vm` 解析 TOML → `AxVM::new`（vm/mod.rs:1435）+ 注册到 `VM_REGISTRY`。

`running()`/`stopping()`/`suspending()`/`stopped()` 都是 `status()` 的**便捷谓词**，每次调用各取一次
machine 锁：`running()` = `Running`（vm/mod.rs:1840）、`stopping()` = `Stopping`（:1845）、
`suspending()` = `Pausing | Paused`（:1850）、`stopped()` = `Stopped`（:1855）。
...
graph LR
    ...
    subgraph EXT["axvm（对外 API 层）"]
        ...
        AXVM["AxVM<br/>start/pause/resume/stop/reset/destroy/status<br/>vm/mod.rs:1760-2475"]
    end
